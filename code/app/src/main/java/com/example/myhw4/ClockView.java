...
public class ClockView extends View {
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() { invalidate(); }},1000);
    }
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        X_end = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(minuteRadian));
        Y_end = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(minuteRadian));
        canvas.drawLine(centerX,centerY,X_end,Y_end,needlePaint);

        // 画秒针

        double secondRadian = Math.toRadians(second * 6 - 90);
        X_end = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(secondRadian));
        Y_end = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(secondRadian));
        ...
        X_end = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(hourRadian));
        Y_end = (float) (centerY + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(hourRadian));
        canvas.drawLine(centerX,centerY,X_end,Y_end,needlePaint);
